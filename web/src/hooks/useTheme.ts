...
const useTheme = (store = null) => {
    ...
    let themeSettings = store.getState().settings.theme
    ...
    const setDarkTheme = (dark = false) => {
        const theme = Object.assign({}, store.getState().settings.theme, {darkTheme: dark})
        store.dispatch({
            type: 'update-settings',
            payload: {settings: {...store.getState().settings, theme}}
        })
